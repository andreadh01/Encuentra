...
const EditPasswordForm = () => {
  ...
  const handleChange = (field, value) => {
    ...
    if (
      newFields.contrasenaNew1 === newFields.contrasenaNew2 &&
      (field === "contrasenaNew1" || field === "contrasenaNew2")
    ) {
      setValidFields({
        ...validFields,
        contrasenaNew1: true,
  ...
  const handleSubmit = async () => {
    ...
    if (
      fields.contrasenaNew1 === fields.contrasenaNew2 &&
      fields.contrasenaNew1 != ""
    ) {
    ...
    if (allFieldsAreValid && allFieldsHaveInput) {
      setIsLoading(true);
      const { data, error } = await supabase.rpc("actualizar_contrasena", {
        iduser: session.user.id,
        current_pass: fields.contrasenaOG,
        new_pass: fields.contrasenaNew1,
      });
      if (error) {
        setValidFields({
          ...validFields,
      ...
      <ScrollView>
        ...
        <View style={styles.userInfoContainer}>
          <View style={styles.inputContainer}>
            ...
            {!validFields.contrasenaOG && fields.contrasenaOG === "" && (
              <Text style={styles.badText}>Por favor, llena este campo</Text>
            )}
          </View>
          <View style={styles.inputContainer}>
            ...
            <PasswordInput
              placeholder=""
              style={
                validFields.contrasenaNew1 ? styles.input : styles.badInput
              }
              handleTextChange={(value) =>
                handleChange("contrasenaNew1", value)
              }
            />
            {!validFields.contrasenaNew1 && fields.contrasenaNew1 != "" && (
              <Text style={styles.badText}>{errorMessage}</Text>
            )}
          ...
          <View style={styles.inputContainer}>
            ...
            <PasswordInput
              placeholder=""
              style={
                validFields.contrasenaNew2 ? styles.input : styles.badInput
              }
              handleTextChange={(value) =>
                handleChange("contrasenaNew2", value)
              }
            />
