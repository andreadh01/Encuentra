...
const EditProfileForm = () => {
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      setOriginalData({
        ...
        email: userProfile.email,
        celular: userProfile.celular,
        image: { uri: profPic, base64: null, width: 0, height: 0 },
      });
    }
  }, [userProfile]);
  ...
  useEffect(() => {
    ...
    if (
      Object.entries(fields).some(([key, value]) => originalData[key] != value)
    ) {
      isNavigationAllowed.current = false;
    }
    ...
    if (
      Object.entries(fields).every(
        ([key, value]) => originalData[key] === value
      ) &&
      image.uri === originalData.image.uri
    ) {
    ...
    if (image.uri != null && image.uri != originalData.image.uri) {
      ...
      if (originalData.image.uri != null) {
        ...
        await supabase.storage
          ...
          .remove([originalRoute]);
      } else {
        new_number = Math.floor(Math.random() * 999999999 + 1);
      }
      //subir nueva imagen
      const extension = image.uri.split(".").pop().toLowerCase();
      const rutaImage = `${session.user.id}/profile${String(
        new_number
      )}.${extension}`;
      ...
      if (data) {
        const ruta = await supabase.storage
          .from("imagenes_perfil_usuarios")
          .getPublicUrl(rutaImage).data.publicUrl;
        const { error } = await supabase
          .from("usuarios")
          .update({ url_imagen_perfil: ruta })
    ...
    if (
      ...
      if (modalMessage.current === "") {
        modalMessage.current =
          "Por favor modifique los campos que desee cambiar";
      } else {
        const newProfile = {
          ...userProfile,
          foto: image.uri,
      ...
      <ScrollView>
        ...
        <View style={styles.userInfoContainer}>
          <View style={styles.inputContainer}>
            ...
            />
            {!validFields.nombres && (
              <Text style={styles.badText}>Ingresa tu(s) nombre(s)</Text>
            )}
          </View>
          <View style={styles.inputContainer}>
            ...
            {!validFields.apellidos && (
              <Text style={styles.badText}>Ingresa tu(s) apellidos(s)</Text>
            )}
          </View>
          <View style={styles.inputContainer}>
          ...
          <View style={styles.inputContainer}>
            ...
            {!validFields.celular && fields.celular === "" && (
              <Text style={styles.badText}>Ingresa un número de celular</Text>
            )}
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              router.navigate("users/EditPassword");
            }}
          >
