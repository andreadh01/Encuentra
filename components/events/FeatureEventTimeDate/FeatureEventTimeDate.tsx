...
const FeatureEventTimeDate: React.FC = () => {
  ...
  const [rangosFechasCobrados, setRangosFechasCobrados] =
    useState(rangosFechasVacios);
  ...
  useEffect(() => {
    if (!(firstDay === UNSET_DATE) && !(lastDay === UNSET_DATE)) {
      ...
      while (currentDay <= lastDayDate) {
        let diferencia_meses = Math.ceil(
          diaEvento.diff(currentDay, "month", true)
        );
        let diferencia_dias = diaEvento.diff(currentDay, "day", true);
        switch (true) {
          case diferencia_meses >= 3:
            rangosFechasCopy["3meses"].push(
              currentDay.format(DISPLAY_DATE_FORMAT)
            );
            break;
          case diferencia_meses >= 2 && diferencia_meses < 3:
            rangosFechasCopy["2meses"].push(
              currentDay.format(DISPLAY_DATE_FORMAT)
            );
            break;
          case diferencia_meses < 2 &&
            diferencia_meses <= 1 &&
            diferencia_dias > 3:
            rangosFechasCopy["1mes"].push(
              currentDay.format(DISPLAY_DATE_FORMAT)
            );
            break;
          case diferencia_dias <= 3 && diferencia_dias != 0:
            rangosFechasCopy["3dias"].push(
              currentDay.format(DISPLAY_DATE_FORMAT)
            );
            break;
          case diferencia_dias === 0:
            rangosFechasCopy["diaevento"].push(
              currentDay.format(DISPLAY_DATE_FORMAT)
            );
            break;
          default:
  ...
  function getOverlap(rangoNuevo: {}, rangosActuales: []) {
    ...
    rangosActuales.forEach((rango) => {
      ...
      if (
        rangoNuevoInicio.isBetween(
          rangoActualInicio,
          rangoActualFinal,
          null,
          "[]"
        ) ||
        rangoNuevoFinal.isBetween(
          rangoActualInicio,
          rangoActualFinal,
          null,
          "[]"
        ) ||
        (rangoNuevoInicio.isBefore(rangoActualInicio) &&
          rangoNuevoFinal.isAfter(rangoActualFinal))
      ) {
        overlap = true;
        rangoOverlap = rango;
      }
      ...
      <ScrollView>
        ...
        <View style={styles.calendarContainer}>
          <MyCalendar
            ...
            onFirstDaySelect={setFirstDay}
            onLastDaySelect={setLastDay}
          />
        </View>
        ...
        <View>
          ...
          <View style={styles.rangoFechaContainer}>
            <View>
              ...
              <Text style={styles.dateButton}>
                {firstDay === UNSET_DATE
                  ? DISPLAY_DATE_FORMAT
                  : dayjs(firstDay).format(DISPLAY_DATE_FORMAT)}
              </Text>
            </View>
            <View>
              ...
              <Text style={styles.dateButton}>
                {lastDay === UNSET_DATE
                  ? DISPLAY_DATE_FORMAT
                  : dayjs(lastDay).format(DISPLAY_DATE_FORMAT)}
              </Text>
            </View>
          </View>

          {firstDay != "YYYY-MM-DD" && lastDay != "YYYY-MM-DD" && (
            <Text style={styles.infoAnticipacion}>
              Tu programación tiene{" "}
              {dayjs(event_date).diff(
                dayjs(firstDay).format(UNSET_DATE),
                "day"
              )}{" "}
          ...
          {firstDay != "YYYY-MM-DD" && lastDay != "YYYY-MM-DD" && (
            <TouchableOpacity
              ...
              onPress={() => {
                setIsDesgloceDiasActive(!isDesgloceDiasActive);
                setIsFlipped(!isFlipped);
              }}
            >
              <DesgloceCostos
                isFlipped={isFlipped}
                text={"Desgloce de costos"}
              />
            </TouchableOpacity>
          )}

          {isDesgloceDiasActive &&
            firstDay != "YYYY-MM-DD" &&
            lastDay != "YYYY-MM-DD" && (
              <View>
                {rangosFechasCobrados["3meses"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    <Text style={styles.rangosText}>
                      {rangosFechasCobrados["3meses"][0]} -{" "}
                      {
                        rangosFechasCobrados["3meses"][
                          rangosFechasCobrados["3meses"].length - 1
                        ]
                      }
                    </Text>
                ...
                {rangosFechasCobrados["2meses"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    <Text style={styles.rangosText}>
                      {rangosFechasCobrados["2meses"][0]} -{" "}
                      {
                        rangosFechasCobrados["2meses"][
                          rangosFechasCobrados["2meses"].length - 1
                        ]
                      }
                    </Text>
                ...
                {rangosFechasCobrados["1mes"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    <Text style={styles.rangosText}>
                      {rangosFechasCobrados["1mes"][0]} -{" "}
                      {
                        rangosFechasCobrados["1mes"][
                          rangosFechasCobrados["1mes"].length - 1
                        ]
                      }
                    </Text>
                ...
                {rangosFechasCobrados["3dias"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    <Text style={styles.rangosText}>
                      {rangosFechasCobrados["3dias"][0]} -{" "}
                      {
                        rangosFechasCobrados["3dias"][
                          rangosFechasCobrados["3dias"].length - 1
                        ]
                      }
                    </Text>
        ...
        {startHour != null && endHour != null && (
          <View>
            <Text style={styles.horasDiariasText}>
              {dayjs(endHour).diff(dayjs(startHour), "hour")} HORAS AL DÍA
            </Text>
            {firstDay != "YYYY-MM-DD" && lastDay != "YYYY-MM-DD" && (
              <TouchableOpacity
                style={{ paddingVertical: 5 }}
                onPress={() => {
                  setIsDesgloceHorasActive(!isDesgloceHorasActive);
                  setIsFlippedHours(!isFlippedHours);
                }}
              >
                <DesgloceCostos
                  isFlipped={isFlippedHours}
                  text={"Desgloce de costos"}
                />
              </TouchableOpacity>
            )}
              ...
              <View>
                {rangosFechasCobrados["3meses"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    ...
                    <Text style={styles.rangosText}>
                      {" "}
                      ${dayjs(endHour).diff(dayjs(startHour), "hour")}.00/día
                    </Text>
                  </View>
                )}
                {rangosFechasCobrados["2meses"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    ...
                    <Text style={styles.rangosText}>
                      {" "}
                      ${dayjs(endHour).diff(dayjs(startHour), "hour") * 2}
                      .00/día
                    </Text>
                  </View>
                )}
                {rangosFechasCobrados["1mes"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    ...
                    <Text style={styles.rangosText}>
                      {" "}
                      ${dayjs(endHour).diff(dayjs(startHour), "hour") * 5}
                      .00/día
                    </Text>
                  </View>
                )}
                {rangosFechasCobrados["3dias"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    ...
                    <Text style={styles.rangosText}>
                      {" "}
                      ${dayjs(endHour).diff(dayjs(startHour), "hour") * 10}
                      .00/día
                    </Text>
                  </View>
                )}
                {rangosFechasCobrados["diaevento"].length > 0 && (
                  <View style={styles.rangosDiasContainer}>
                    ...
                    <Text style={styles.rangosText}>
                      ${dayjs(endHour).diff(dayjs(startHour), "hour") * 20}
                      .00/día
                    </Text>
                  </View>
                )}
        ...
        <Text style={styles.footerText}>
          Tu evento se verá en la sección de destacados en los rangos de fecha y
          hora que ingresaste
        </Text>
        {rangosFechasActuales != null && rangosFechasActuales.length > 0 && (
          <View>
            <TouchableOpacity
              ...
              onPress={() => {
                setIsDesgloceRangeActive(!isDesgloceRangeActive);
                setIsFlippedRange(!isFlippedRange);
              }}
            >
              <DesgloceCostos
                isFlipped={isFlippedRange}
                text={"Tu evento está destacado en estas fechas:"}
              />
            </TouchableOpacity>
            {isDesgloceRangeActive && (
              <View>
                {rangosFechasActuales.map((rango, index) => {
                  let fecha_inicio = dayjs(rango["fecha_inicio"]).format(
                    "DD/MM/YYYY"
                  );
                  let fecha_final = dayjs(rango["fecha_final"]).format(
                    "DD/MM/YYYY"
                  );
                  let date = new Date(`2024-03-11T${rango["hora_inicio"]}`);
                  let hora_inicial = date.toLocaleString("es", {
                    hour: "numeric",
                    minute: "numeric",
                    hour12: true,
                  });
                  date = new Date(`2024-03-11T${rango["hora_final"]}`);
                  let hora_fin = date.toLocaleString("es", {
                    hour: "numeric",
                    minute: "numeric",
                    hour12: true,
                  });
                  ...
                  return (
                    <View style={styles.rangosDiasContainer}>
                      <Text key={"fecha" + index} style={styles.rangosText}>
                        {fecha_inicio} a {fecha_final}
                      </Text>
                      <Text key={"hora" + index} style={styles.rangosText}>
                        {hora_inicial} a {hora_fin}
                      </Text>
                    </View>
                  );
                })}
          ...
          <View style={styles.containerContainer}>
            ...
            <View style={styles.empalmeContainer}>
              ...
              <Text style={styles.warningText}>
                {" "}
                Tu evento ya está destacado entre estas fechas:
              </Text>
              <Text style={styles.warningText}>
                {" "}
                {dayjs(rangoOverlap["fecha_inicio"]).format(
                  "DD/MM/YYYY"
                )} a {dayjs(rangoOverlap["fecha_final"]).format("DD/MM/YYYY")}{" "}
              </Text>
            </View>
          </View>
        )}
        ...
        {!validInputs && (
          <View style={styles.nextButtonContainer}>
            <NavButton
              type={"invalid"}
              handlePress={() => console.log("perro baboso")}
            />
          </View>
        )}
